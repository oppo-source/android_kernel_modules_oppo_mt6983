...
static const struct typec_switch_reg_val dio4483_i2c_reg_reset[] = {
	//register rewrite after abnormal status(0x17=01)
	{0x2e, 0x8f},
	{0x2f, 0x45},
	{0x2e, 0x72},
	{0x0f, 0x00},
	...
	if (switch_priv->plug_state) {
		...
		if (switch_priv->vendor == DIO4480) {
			...
			if (jack_status == 0x01) {
				...
				//After the abnormal state(0x17=01), go through the headphone detection process again
				regmap_write(switch_priv->regmap, DIO4483_REG_RESET, 0x01);//reset DIO4483
				usleep_range(1000, 1005);
				for (i = 0; i < ARRAY_SIZE(dio4483_i2c_reg_reset); i++) {
					regmap_write(switch_priv->regmap, dio4483_i2c_reg_reset[i].reg,
					dio4483_i2c_reg_reset[i].val);
				}
				...
				if (jack_status == 0x01) {
					dev_info(dev, "V1.2 %s: error status,swap MIC_GND\n", __func__);
					regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SELECT, 0x00);
...
#ifdef OPLUS_ARCH_EXTENDS
		/*2024/07/09, supporting type-c headphone detect bypass*/
		...
		if (switch_priv->vendor == DIO4480) {
			...
			regmap_write(switch_priv->regmap, 0x1c, 0x2a);
			regmap_write(switch_priv->regmap, 0x1f, 0x0f);
			regmap_write(switch_priv->regmap, 0x2e, 0x8f);
			regmap_write(switch_priv->regmap, 0x2f, 0x45);
			regmap_write(switch_priv->regmap, 0x2e, 0x72);
			regmap_write(switch_priv->regmap, 0x0f, 0x00);
			regmap_write(switch_priv->regmap, 0x10, 0x00);
			regmap_write(switch_priv->regmap, 0x0e, 0x0b);
			regmap_write(switch_priv->regmap, 0x0d, 0x0f);
			regmap_write(switch_priv->regmap, 0x21, 0x0f);
			// */v1.2 end
...
//2020/11/04, support mic and ground switch to fix headset detect bug
	...
	case TYPEC_SWITCH_MIC_GND_SWAP:
		if (switch_priv->vendor == DIO4480) {
			...
			if (typec_switch_status) {
				pr_info("%s - switch event: %d delay 0ms\n", __func__, event);
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SETTINGS, 0x87);
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SELECT, 0x07);
				regmap_write(switch_priv->regmap, DIO4483_REG_SWITCH_SETTINGS, 0x9f);
				typec_switch_status = 0;
			}
		} else {
...
#ifdef OPLUS_ARCH_EXTENDS
	/*2023/07/09, supporting type-c headphone detect bypass*/
	ret = of_property_read_u32(dNode,
