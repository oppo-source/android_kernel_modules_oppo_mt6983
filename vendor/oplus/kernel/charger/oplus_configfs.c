				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	static bool is_fastchg_first_check_type = true;
	...
	if (vooc_online) {
		...
		} else if ((!subtype) && (is_fastchg_first_check_type)) {
			is_fastchg_first_check_type = false;
			msleep(300);
		} else {
		...
		is_fastchg_first_check_type = true;
